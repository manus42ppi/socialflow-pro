...
// Renders a Lucide icon by name string, with fallback to Flag
...
export default function CampaignsPage(){
  ...
  const [form, setForm] = useState({ name:"", icon:"Target", color:C.accent, description:"" });
  const [sel, setSel] = useState(null);
  const { order, dragId, setDragId, overId, setOverId, drop } = useSections("campaigns","default",["active","list"]);

  const create = () => {
    if (!form.name.trim()) return;
    setCampaigns(p => [...p, { id:uid(), ...form }]);
    setShowNew(false);
    setForm({ name:"", icon:"Target", color:C.accent, description:"" });
  };
  const del = id => {
    if (window.confirm("Kampagne löschen?")) {
    ...
    <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4 }}>
        <Btn size="sm" onClick={() => setShowNew(s => !s)}>
          <LucideIcons.Plus size={13} strokeWidth={2}/>
          {showNew ? "Schließen" : "Neue Kampagne"}
        </Btn>
      </div>

      {showNew && (
        <Card style={{ padding:"14px 16px", marginBottom:4 }}>
          <div style={{ fontWeight:600, fontSize:13, marginBottom:12 }}>Neue Kampagne</div>
          <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
                ...
                {CAMP_ICONS.map(name => {
                  ...
                  return (
                    <button key={name} onClick={() => setForm(f => ({ ...f, icon:name }))}
                      ...
                      <CampIcon name={name} size={16}
                        color={on ? form.color : C.textMute}
            ...
            <TIn label="Name" placeholder="z.B. Sommer, Olympia, Ostern…"
              value={form.name} onChange={e => setForm(f => ({ ...f, name:e.target.value }))}/>

            <TIn label="Beschreibung (optional)" placeholder="Kurze Info…"
            ...
            <div>
              ...
              <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
                {CAMP_COLORS.map(clr => (
                  <button key={clr} onClick={() => setForm(f => ({ ...f, color:clr }))}
                    style={{
                      width:24, height:24, borderRadius:"50%", background:clr,
                      border:"none", cursor:"pointer",
                      outline: form.color === clr ? `2.5px solid ${clr}` : "none",
            ...
            <div style={{ display:"flex", gap:8 }}>
              <Btn variant="secondary" size="sm" onClick={() => setShowNew(false)}
                style={{ flex:1, justifyContent:"center" }}>Abbrechen</Btn>
              <Btn size="sm" onClick={create} style={{ flex:2, justifyContent:"center" }}>
                <LucideIcons.Check size={12} strokeWidth={2.5}/>Erstellen
              </Btn>
            </div>
          </div>
        </Card>
      ...
      {campaigns.length === 0 && !showNew && (
        <div style={{ textAlign:"center", padding:"40px 16px", color:C.textMute }}>
          <LucideIcons.Flag size={36} strokeWidth={1} style={{ margin:"0 auto 10px", display:"block" }}/>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(240px,1fr))", gap:8 }}>
        {campaigns.map(c => {
          ...
          const isS = sel === c.id;
          // Backwards-compat: old campaigns have `emoji` string instead of `icon`
          const iconName = c.icon || "Flag";
          return (
            <div key={c.id} onClick={() => setSel(isS ? null : c.id)}
              style={{
                background: isS ? c.color + "12" : C.surface,
                borderRadius:10,
                border:`1.5px solid ${isS ? c.color : C.border}`,
              ...
              <div style={{ display:"flex", alignItems:"center", gap:10 }}>
                {/* Monochrome icon in campaign color */}
                <div style={{
                  ...
                  <CampIcon name={iconName} size={17} color={c.color} strokeWidth={1.8}/>
                </div>
                ...
                <button onClick={e => { e.stopPropagation(); del(c.id); }}
                  style={{ background:"none", border:"none", color:C.textMute, cursor:"pointer", padding:4 }}>
                  <LucideIcons.Trash2 size={14} strokeWidth={IW}/>
                </button>
              </div>
              {c.description && (
                <div style={{
                  fontSize:12, color:C.textSoft, marginTop:6, paddingTop:6,
  ...
  const listContent = selC ? (
    <>
      <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:14 }}>
        <div style={{
          width:44, height:44, borderRadius:11,
          background: selC.color + "18",
          display:"flex", alignItems:"center", justifyContent:"center",
        }}>
          <CampIcon name={selC.icon || "Flag"} size={22} color={selC.color} strokeWidth={1.7}/>
        </div>
        <div>
          <div style={{ fontWeight:900, fontSize:18, color:C.text }}>{selC.name}</div>
          <div style={{ fontSize:13, color:C.textSoft }}>{cPosts.length} Posts</div>
        </div>
      </div>
      {cPosts.length === 0 ? (
        <div style={{ textAlign:"center", padding:"60px 20px", color:C.textMute }}>
          <LucideIcons.Send size={36} strokeWidth={1} style={{ margin:"0 auto 12px", display:"block" }}/>
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))", gap:10 }}>
          {cPosts.map(p => (
            <div key={p.id} onClick={() => onEditPost(p)}
              ...
              <div style={{
                fontSize:12, color:C.textSoft, marginBottom:8,
    ...
    <div style={{ textAlign:"center", padding:"48px 20px", color:C.textMute }}>
      <LucideIcons.Flag size={48} strokeWidth={1} style={{ margin:"0 auto 14px", display:"block" }}/>
      ...
      {order.map(id => {
        ...
        return (
          <SecCard key={id} id={id} title={w.title} right={w.right}
            dragId={dragId} overId={overId} setDragId={setDragId} setOverId={setOverId} drop={drop}>
            {w.content}
          </SecCard>
        );
      })}
