    ...
    <div
      ...
      <div
        ...
        <div style={{ padding:"16px 18px 20px" }}>
          ...
          <div style={{ fontSize:13.5, color:C.text, lineHeight:1.6, marginBottom:14, whiteSpace:"pre-line" }}>{post.caption}</div>
          {isWa && post.fullText && (
          ...
          {!isWa ? (
            <div style={{ display:"flex", alignItems:"center", gap:14, padding:"12px 14px", background:C.bg, borderRadius:10 }}>
              <span style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, color:C.textMid }}><Heart size={14} strokeWidth={2} color="#EF4444"/><strong>{post.likes}</strong></span>
              <span style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, color:C.textMid }}><MessageSquare size={14} strokeWidth={2}/><strong>{post.comments}</strong></span>
              <span style={{ display:"flex", alignItems:"center", gap:5, fontSize:12, color:C.textMid }}><Share2 size={14} strokeWidth={2}/><strong>{post.shares}</strong></span>
              <span style={{ marginLeft:"auto", fontSize:12, fontWeight:800, color:"#16A34A", display:"flex", alignItems:"center", gap:4 }}><TrendingUp size={13} strokeWidth={2.5}/>{post.growth}</span>
            </div>
          ) : (
            <div style={{ padding:"10px 14px", background:C.bg, borderRadius:10 }}>
              <div style={{ fontSize:11.5, color:C.textMid, marginBottom:4 }}>Weiterleitungsrate: <strong style={{ color:ENG_CLR[post.fwds]||C.textMute }}>{post.fwds}</strong></div>
...
export default function TrendsPage() {
  ...
  return (
    <div style={{ flex:1, overflow:"hidden", display:"flex", flexDirection:"column", background:"#F9FAFB", fontFamily:FONT }}>
      ...
      {/* Header */}
      <div style={{ padding:"14px 20px 12px", flexShrink:0, borderBottom:`1px solid ${C.borderLight}`, background:"#fff", display:"flex", alignItems:"center", justifyContent:"space-between" }}>
        <div>
          <div style={{ fontSize:11.5, color:C.textMute, display:"flex", alignItems:"center", gap:8 }}>
            {lastUpdate ? (
              <>
